...
This is an Advent of Code 2025 solutions repository using Groovy with Gradle. Each day's puzzle has its own implementation file and corresponding Spock test specifications.
...
```bash
./gradlew test --tests Day1Specification
./gradlew test --tests "Day1Specification.test case name"
```

**Run a single day's solution (from src/main/groovy/):**
```bash
groovy src/main/groovy/day1.groovy
```
...
- **src/main/groovy/**: Solution implementations
  - Each file is named `day{N}.groovy`
...
2. **Solution class** containing the core logic
...
1. Create `src/test/groovy/Day{N}Specification.groovy` with example test cases
2. Create `src/main/groovy/day{N}.groovy` with problem description
